...
namespace LightBulb.Services
{
    ...
    public abstract class WinApiServiceBase : IDisposable
    {
        ...
        {
            var handle = NativeMethods.SetWinEventHookInternal(eventId, eventId, IntPtr.Zero, handler, processId, threadId, flags);
        ...
        {
            if (!NativeMethods.UnhookWinEventInternal(handle))
            {
                CheckLogWin32Error();
        ...
        protected void UnregisterAllWinEvents()
        {
            ...
            {
                if (!NativeMethods.UnhookWinEventInternal(hook.Key))
                {
                    CheckLogWin32Error();
                    Debug.WriteLine("Could not unregister WinEventHook", GetType().Name);
                }
